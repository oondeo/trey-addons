...
class TestHrTimesheetStats(common.TransactionCase):
    ...
    def test_send_mail_correct_data_without_old_analytic_line(self):
        ...
        self.assertIn(
            "Total tasks - <strong>%s</strong>" % (employee_total_works),
            self.employee1.message_ids[0].body,
        )
        self.assertIn(
            "Total hours - <strong>%s</strong>" % (employee_unit_amount),
            self.employee1.message_ids[0].body,
        )
        self.assertIn(
            "Total real hours - <strong>%s</strong>" % (employee_real_hours),
            self.employee1.message_ids[0].body,
        )
        self.employee2.send_mail()
        self.assertIn("Dear Employee2", self.employee2.message_ids[0].body)
        self.assertIn(
            "Total tasks - <strong>%s</strong>"
            % (self.employee2.total_works()),
            self.employee2.message_ids[0].body,
        )
        self.assertIn(
            "Total hours - <strong>%s</strong>"
            % (self.employee2.total_unit_amount()),
            self.employee2.message_ids[0].body,
        )
        self.assertIn(
            "Total real hours - <strong>%s</strong>"
