...
class TestSaleReturn(TransactionCase):
    ...
    def test_invoice_with_lines(self):
        ...
        self.assertEquals(
            first.date_invoice, fields.Date.today() + relativedelta(months=0)
        )
        self.assertEquals(
            second.date_invoice, fields.Date.today() + relativedelta(months=1)
        )
        self.assertEquals(
            third.date_invoice, fields.Date.today() + relativedelta(months=2)
        )
