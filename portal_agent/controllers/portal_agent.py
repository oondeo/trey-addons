...
class PortalAgentCustomerPortal(CustomerPortal):
    ...
    def _prepare_portal_layout_values(self):
        ...
        agent_customer_ids = (
            ...
            .sudo()
            .search(self._get_agent_customers_domain(), order="name asc")
        )
    ...
    def portal_my_customers(self):
        agent_customers = (
            request.env["res.partner"]
            .sudo()
            .search(self._get_agent_customers_domain())
        )
        values = {
            "page_name": "agent_customers",
            "agent_customers": agent_customers,
        }
        return request.render("portal_agent.portal_my_customers", values)

    @http.route(
        ["/my/customer/<int:agent_customer_id>"],
        type="http",
        auth="user",
        website=True,
    )
    def portal_my_customer(self, agent_customer_id):
        ...
        agent_order_lines = (
            request.env["sale.order.line.agent"]
            .sudo()
            .search(self._get_agent_customer_orders_domain(agent_customer_id))
        )
        ...
        return request.render("portal_agent.portal_my_customer", values)
    ...
    def portal_my_customer_orders(self, agent_customer_id):
        ...
        orders = (
            agent_order_lines
            and agent_order_lines.mapped("object_id.order_id")
            or False
        )
        agent_customer = (
            request.env["res.partner"].sudo().browse(agent_customer_id)
        )
        values = {
        ...
        return request.render("portal_agent.portal_my_customer_orders", values)
    ...
    def portal_my_customer_products(self, agent_customer_id):
        ...
        dates = agent_order_lines.mapped(
            "object_id.order_id.confirmation_date"
        )
        ...
        for line in agent_order_lines:
            ...
            products[code]["years"][order_year] += int(
    ...
    def _get_sale_quotation_domain(self):
        partner_id = (
            request.env.user.sudo().partner_id.commercial_partner_id.id
        )
        return [
