...
class AccountInvoiceMakeAdvance(models.TransientModel):
    ...
    def _get_percent_values(self):
        ...
        try:
            ...
            raise UserError(
                _("You must enter valid numerical values separated by + sign")
            )
        if sum(vals) >= 100:
            raise UserError(
                _("The sum of the percentages is equal or greater than 100%")
            )
        return vals
        ...
        if sum(percents) + invoice.percent_advanced >= 100:
            raise UserError(
                _(
                    ...
                    "invoice"
                    % (
                        invoice.number or invoice.name,
                        invoice.percent_advanced,
                        sum(percents),
                    )
                )
            )
    ...
    def _create_invoice_line(
        self, invoice, product, price_unit, percent, advance_line=False
    ):
