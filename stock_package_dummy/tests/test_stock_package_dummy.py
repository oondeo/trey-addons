...
class TestStockPackageDummy(TransactionCase):
    ...
    def create_inventory(
        self, product, location, qty, lot_id=False, package_id=False
    ):
    ...
    def test_wizard_create_stock(self):
        ...
        self.assertTrue(
            all(
                [
                    b.startswith(wizard.dummy_id.barcode_prefix)
                    for b in barcodes
                ]
            )
        )
    ...
    def test_wizard_create_stock_picking(self):
        ...
        self.assertTrue(
            any(["The product is not required" in e for e in errors])
        )
        wizard_create.write(
            {
                "barcodes": barcodes[0],
