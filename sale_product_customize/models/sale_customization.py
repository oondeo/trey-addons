...
class SaleCustomization(models.Model):
    ...
    sale_line_count = fields.Integer(
        string="Lines count", compute="_compute_sale_line_count"
    )
    ...
    def action_pending_logo(self):
        if not self.line_ids:
            raise exceptions.Warning(
                _("You must add fill in at least one line to customization!")
            )
    ...
    def action_in_design(self):
        ...
        for line in self.line_ids:
            if not line.image:
                ...
                raise exceptions.Warning(
                    _("You must fill in the logo's images before continuing!")
                )
