...
class DeliveryCarrier(models.Model):
    ...
    def _get_tracking_link_gls(self, picking):
        url_spain = (
            ...
            .sudo()
            .get_param("delivery_carrier.tracking_link.gls_spain")
        )
        url_int = (
            self.env["ir.config_parameter"]
            .sudo()
            .get_param("delivery_carrier.tracking_link.gls_international")
        )
        ...
        if (
            not picking
            or not partner_id
            or "%s" not in url_spain
            or "%s" not in url_int
        ):
            return ""
        if partner_id.country_id.phone_code == 34:
            return (
                (carrier_tracking_ref and partner_id.zip)
                and url_spain % (carrier_tracking_ref, partner_id.zip)
                or ""
            )
        else:
            return (
