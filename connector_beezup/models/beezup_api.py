...
class BeezupApi(models.Model):
    ...
    def get_beezup_order_data(self, order=False):
        ...
        if res_dict["paginationResult"]["pageCount"] > 1:
            for page in range(page, res_dict["paginationResult"]["pageCount"]):
    ...
    def sync_beezup_order_state(self, picking):
        ...
        delivery_carrier_link = (
            picking.carrier_id
            and picking.carrier_id.fixed_get_tracking_link(picking)
            or False
        )
        data = {
            ...
            "order_Shipping_ShipperTrackingNumber": picking.carrier_tracking_ref,
        ...
        response = requests.post(
            ...
            % (
                api_url,
                order_data["marketplaceTechnicalCode"],
                order_data["accountId"],
                order_data["beezUPOrderId"],
            ),
            headers={
                "Ocp-Apim-Subscription-Key": company.beezup_token,
                "content-type": "application/json",
    ...
    def process_order(self, bz_order, order):
        ...
        if not order_bz_status:
            errors.append(
                ...
                % (order.origin, bz_status)
            )
            return errors
        if order.state_beezup == order_bz_status:
            return errors
        order.write(
            {"state_beezup": bz_status, "etag_beezup": bz_order["etag"]}
        )
        if order_bz_status == "cancelled":
            ...
            if not any(
                ...
                return_piking_error = self.create_return_pickings(
                    done_pickings
                )
    ...
    def create_beezup_partner(self, bz_order):
        ...
        partner_data = {
            "city": zip
            and zip.city_id.name
            or bz_order.get("order_Buyer_AddressCity", ""),
            ...
            "zip": zip
            and zip.name
            or bz_order.get("order_Buyer_AddressPostalCode", ""),
            "zip_id": zip.id,
    ...
    def create_beezup_partner_shipping(self, bz_order, parent):
        ...
        partner_ship_data = {
            "city": zip
            and zip.city_id.name
            or bz_order.get("order_Shipping_AddressCity", ""),
            "city_id": zip and zip.city_id.id or False,
            "country_id": zip and zip.city_id.country_id.id or country_id,
            "name": bz_order["order_Shipping_AddressName"],
            "parent_id": parent.id,
            ...
            "zip": zip
            and zip.name
            or bz_order.get("order_Shipping_AddressPostalCode", ""),
            "zip_id": zip.id,
        }
                ...
                info = self.env["product.supplierinfo"].search(
                    [
                        (
                            "product_code",
                            "=",
                            item["orderItem_MerchantProductId"],
                        ),
                    ],
                    limit=1,
                )
    ...
    def create_beezup_sale_order(self, bz_order):
        ...
        crm_team_partner_id = (
            crm_team
            and crm_team.partner_id
            and crm_team.partner_id.id
            or False
        )
        if not crm_team:
        ...
        order_data = {
            ...
            "partner_invoice_id": (
                company.beezup_force_partner
                and company.beezup_parent_partner_id.id
                or crm_team_partner_id
                or partner.id
            ),
            "partner_shipping_id": (
        ...
        if company.sale_number_overwrite:
            order_data["name"] = "%s%s" % (
                crm_team.beezup_prefix_sale_name or "",
                bz_order_id,
            )
    ...
    def confirm_sale_order(self, bz_order, sale_order):
        ...
        if invoice_policy != ["order"]:
            msg_error = _(
                ...
                " manually."
            ) % (sale_order.origin, invoice_policy)
        ...
        self.register_payment(
            bz_payment_method,
            invoice,
            company.beezup_journal_payment_id,
            payment_date,
        )
        if invoice.state != "paid":
            msg_error = (
                _(
                    "The invoice associated with the order with origin '%s' has "
                    "not been paid correctly, check manually."
                )
                % sale_order.origin
            )
            errors.append(msg_error)
    ...
    def import_beezup_orders(self):
        ...
        for order_count, bz_order in enumerate(beezup_orders):
            bz_order_id = bz_order["order_MarketplaceOrderId"]
            _log.info(
                "%s/%s %s" % (order_count + 1, len(beezup_orders), bz_order_id)
            )
        ...
        _log.info(
            "Beezup import complete, %s errors, %s skiped"
    ...
    def sync_beezup_orders_state(self):
        ...
        for order_count, order in enumerate(orders_done):
            _log.info(
                "%s/%s %s" % (order_count + 1, len(orders_done), order.origin)
            )
            ...
            if order_errors:
                ...
                bz_status = orders[0]["order_Status_BeezUPOrderStatus"].upper()
                order_bz_status = self.get_beezup_statuses().get(bz_status)
                if not order_bz_status:
                    msg_error = _(
