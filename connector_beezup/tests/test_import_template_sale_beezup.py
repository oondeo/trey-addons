...
class TestImportTemplateSaleBeezup(TransactionCase):
    ...
    def test_import_create_ok(self):
        ...
        wizard = self.env["import.file"].create(
            {
                "template_id": self.env.ref(
                    "connector_beezup.template_sale_beezup"
                ).id,
                "file": file,
        ...
        sale_beezup = (
            self.env["import.template.sale_beezup"]
            .with_context(wizard_id=wizard.id)
            .create(
                {
        ...
        self.assertEquals(sales_1.partner_id.street, "Calle Cañada 10")
        ...
        self.assertEquals(sales_1.fiscal_position_id, self.fiscal_position_fr)
        ...
        self.assertEquals(sales_2.partner_id.name, "Antonia Pérez")
    ...
    def test_import_default_wizard_values_create_ok(self):
        ...
        self.assertEquals(sales_1.pricelist_id, self.default_pricelist)
        ...
        self.assertEquals(
            sales_1.invoice_ids.payment_ids.journal_id, default_journal_payment
        )
        partner_2 = self.env["res.partner"].search(
            [
                ("name", "=", "Antonia Pérez"),
    ...
    def test_import_order_id_empty_create_error(self):
        ...
        self.assertIn(
            _("2: Column 'order_id' cannot be empty."), wizard.line_ids[0].name
        )
        sales_1 = self.env["sale.order"].search(
    ...
    def test_import_partner_id_empty_create_error(self):
        ...
        self.assertEquals(wizard.total_error, 1)
        self.assertIn(
            _("2: Partner empty not found."), wizard.line_ids[0].name
        )
        sales_1 = self.env["sale.order"].search(
            [
                ("origin", "=", "Beezup sale number: 407-3315028-8261943"),
    ...
    def test_import_default_code_empty_create_error(self):
        ...
        self.assertEquals(wizard.total_error, 1)
        self.assertIn(
            _("2: Default code empty not found."), wizard.line_ids[0].name
        )
        sales_1 = self.env["sale.order"].search(
            [
                ("origin", "=", "Beezup sale number: 407-3315028-8261943"),
    ...
    def test_import_duply_product_create_error(self):
        ...
        self.assertEquals(wizard.total_error, 1)
        self.assertIn(
            _("2: More than one product found for 92385."),
    ...
    def test_import_order_with_serveral_lines_create_ok(self):
        ...
        for ship_line in ship_line_prods_52469_52470:
            self.assertEquals(ship_line.product_id.name, "Shipping costs")
    ...
    def test_new_fields_error(self):
        ...
        self.assertIn(
            _(
                ...
                "no defined function to convert it."
            ),
            wizard.line_ids[0].name,
        )
        self.assertIn(
    ...
    def test_import_new_state_id_without_country_create_ok(self):
        ...
        self.assertIn(
            _("2: No fiscal position found for country code '00'"),
            wizard.line_ids[1].name,
        )
        sales_1 = self.env["sale.order"].search(
            [
        ...
        self.assertIn(
            _("2: Country code empty not found; 'No country' is assigned."),
            wizard.line_ids[1].name,
        )
        wizard.with_context(
            wizard_id=wizard.id
    ...
    def test_import_empty_state_id_empty_country_id_create_ok(self):
        fname = self.get_file("sample_empty_state_id_empty_country_id.xlsx")
        file = base64.b64encode(open(fname, "rb").read())
        wizard = self.env["import.file"].create(
            {
                "template_id": self.env.ref(
        ...
        self.assertIn(
            _("2: State name empty not found; not assigned."),
    ...
    def test_import_empty_no_required_fields_create_ok(self):
        ...
        self.assertEquals(wizard.total_error, 0)
        self.assertIn(
            _("2: Email data not found; not assigned"), wizard.line_ids[0].name
        )
        self.assertIn(
            _("2: Street data not found; not assigned"),
            wizard.line_ids[1].name,
        )
        self.assertIn(
            _("2: City data not found; not assigned"), wizard.line_ids[2].name
        )
        self.assertIn(
            _("2: Zip data not found; not assigned"), wizard.line_ids[3].name
        )
        sales_1 = self.env["sale.order"].search(
            [
                ("origin", "=", "Beezup sale number: 407-3315028-8261943"),
    ...
    def test_import_line_canceled_several_lines(self):
        ...
        self.assertEquals(len(sales_2), 1)
        self.assertEquals(sales_2.partner_id.name, "Amparo García")
        self.assertFalse(sales_2.partner_id.parent_id)
        self.assertEquals(sales_2.partner_invoice_id, sales_2.partner_id)
        self.assertEquals(sales_2.partner_shipping_id, sales_2.partner_id)
    ...
    def test_country_fiscal_position(self):
        ...
        self.assertIn(
            _("4: No fiscal position found for country code '00'."),
            wizard.line_ids[2].name,
        )
        self.assertIn(
            _("5: No fiscal position found for country code '00'."),
        ...
        self.assertIn(
            _("4: Country code empty not found; 'No country' is assigned."),
            wizard.line_ids[1].name,
        )
        self.assertIn(
            _(
        ...
        self.assertEquals(sales_4.partner_id.name, "Amparo García")
    ...
    def test_import_phone_mobile(self):
        ...
        self.assertEquals(wizard.total_error, 0)
        self.assertIn(
            _("4: Phone and mobile data not found; not assigned."),
            wizard.line_ids[0].name,
        )
        sales_1 = self.env["sale.order"].search(
            [
