...
class TestBeezupExporter(TransactionCase):
    def setUp(self):
        ...
        (
            ...
            self.fname_update_stock_us_lock,
        ) = self.generate_lang_files("en_US")
        (
            self.fname_es,
            self.fname_es_lock,
            self.fname_update_stock_es,
            self.fname_update_stock_es_lock,
        ) = self.generate_lang_files("es_ES")
        (
            self.fname_pt,
            self.fname_pt_lock,
            self.fname_update_stock_pt,
            self.fname_update_stock_pt_lock,
        ) = self.generate_lang_files("pt_PT")
    ...
    def check_beezup_generate_company(self, test, company_id=None):
        ...
        if test == "type_1":
            ...
            self.assertEquals(
                ...
                "%s, %s" % (str(self.product1.id), str(self.product2.id)),
            )
            fname, content = product_obj.beezup_generate_file(
                "standard", company_id=company_id
            )
            ...
            fname_update_stock_pt, content = product_obj.beezup_generate_file(
            ...
            self.force_future_write_date(move)
            fname_update_stock_us, content = product_obj.beezup_generate_file(
                "update_stock", company_id=company_id
            )
            self.assertEquals(
    ...
    def test_beezup_generate_file_params_several_langs_update_stock_no_company(
        ...
        self.assertEquals(move.product_uom_qty, 10)
        self.force_future_write_date(move)
        fname_update_stock_es, content = self.env[
            "product.product"
        ].beezup_generate_file("update_stock", lang="es_ES")
    ...
    def test_beezup_image_get_one_image(self):
        ...
        self.product1.product_tmpl_id.write(
            {
                "image": image,
                "product_image_ids": [(0, 0, {"image": i}) for i in images],
            }
        )
