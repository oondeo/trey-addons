...
class TestBeezupBaseException(TransactionCase):
    def setUp(self):
        ...
        self.activity_type_warn_id = self.ref(
    ...
    def generate_empty_file(self, fname):
        ...
        fname = os.path.join(tools.config.filestore(self.env.cr.dbname), fname)
        df.to_csv(fname, header=True, index=False, sep=";")

    def create_base_exception(
        self, function_name, function_params, action_to_launch, company_id=None
    ):
    ...
    def test_beezup_get_file_last_exception_file_type_unknown_no_company(self):
        ...
        base_exception = self.create_base_exception(
            "beezup_get_file_last",
            params,
            "schedule_activity_exception",
            self.company.id,
        )
    ...
    def test_beezup_image_get_without_image(self):
        ...
        base_exception = self.create_base_exception(
            "image_get", params, "schedule_activity_exception", self.company.id
        )
