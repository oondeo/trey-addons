...
class SaleOrder(models.Model):
    ...
    def action_confirm(self):
        ...
        if session_required and self.session_id.state != "open":
            ...
            if not session:
                raise UserError(
                    _("You must open a sale session or change the sale team.")
                )
    ...
    def session_pay(self, amount, payment_journal):
        ...
        journal = (
            ...
            and self.session_id.team_id.invoice_journal_ids[0]
            or False
        )
    ...
    def open_confirm_and_pay(self):
        ...
        if not self.session_id:
            raise UserError(
                _("For confirm and paid must have a sale session.")
            )
        ...
        action = self.env.ref(
            "sale_session.sale_order_confirm_and_pay_action"
        ).read()[0]
