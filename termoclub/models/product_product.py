...
class ProductProduct(models.Model):
    ...
    def termoclub_check_stock(self):
        ...
        res = termoclub.get_product(
            client=client, product=supplier_info.product_code
        )
        if res.find("error"):
        ...
        if not self.env.user.has_group(
            "base.user_admin"
        ) or not self.env.user.has_group("purchase.group_purchase_manager"):
            return msg
        ...
        self.product_tmpl_id.message_post(
            body=_(
                "The supplier price is changed, old: %s new: %s <p>"
