    ...
    def action_sent_to_termoclub(self):
        for order in self:
            ...
            xml_order = {
                ...
                "TIPENT": "D",
                "DIRENV": order.picking_type_id.warehouse_id.termoclub_address_id,
                "REFER": "P",
            }
            for line in lines:
                cod_art = (
            ...
            if res.find("R1") and res.find("R1").text is not None:
                if res.find("R2") is not None:
                    msg += (
                        _(
                            "<b>Send order error</b><p><ul><li><b>ID:</b> "
                ...
                if res.find("R3") is not None:
                    ...
                    msg += _(
                        ...
                        "%s</ul>"
                    ) % (res.find("R3").text, res.find("R1").text)
            else:
                msg += _(
                    "<b>Send order error</b><p><ul><li><b>Message:</b></li> "
                    "%s</ul>"
                ) % (et.tostring(res, method="html"))
