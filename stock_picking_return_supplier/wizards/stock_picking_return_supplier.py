    ...
    def add_stock_move_lines_to_confirm_lines_resume(
        ...
        for line in lines:
            ...
            moves_in = order.picking_ids.filtered(
                lambda p: p.location_dest_id == self.location_id
                and (p.location_id != self.location_id)
            )
            ...
            moves_out = order.picking_ids.filtered(
                lambda p: p.location_id == self.location_id
            ...
            wizard_line = self.env[
                "stock.picking.return.supplier.line"
            ].browse(line_ref)
    ...
    def button_assign(self):
        ...
        for line in self.line_ids:
            ...
            lines = pickings.mapped("move_lines").filtered(
                ...
                and (m.sale_line_id.order_id.partner_id == line.partner_id)
            )
...
class StockPickingReturnSupplierLine(models.TransientModel):
    ...
    wizard_id = fields.Many2one(
        comodel_name="stock.picking.return.supplier", string="Wizard"
    )
