        ...
        self.env["ir.attachment"].create(
            {
                ...
                "res_id": picking.id,
                "mimetype": shipment.payload["Label"]["FileContents"][
                    "FileType"
                ],
            }
        )
    ...
    def _amazon_send_shipping(self, picking):
        ...
        try:
            ...
            if ex.error[0]["code"] == "InvalidInput":
                raise exceptions.ValidationError(
                    _(
                        "Order Id %s not exists in Amazon, please change client "
        ...
        packaging = packaging_get(
            package_max, {k: v["dimension"] for k, v in items.items()}
        )
        item_lists = []
        dimensions = []
        ...
        for package, dimension in packaging:
            item_lists.append(
                [
                    {
                        "OrderItemId": items["%s::0" % key]["item"][
                            "OrderItemId"
                        ],
                        "Quantity": qty,
                    }
        ...
        if not item_lists:
            raise exceptions.ValidationError(
                _("Without packages, I can not continue for security")
            )
        res = {
            "tracking_number": "",
            "exact_price": 0,
