...
class ProductProductStockRotation(models.Model):
    ...
    def calculate_rotation(
        ...
        if not float_is_zero(price, precision_rounding=2):
            result["rate_price"] = (qty_sale * price) / (
                average_inventory * price
            )
        else:
            result["rate_price"] = 0.00
        return result

    def compute_quantities_by_date(
        self, date_from=None, date_to=None, company=None
    ):
        if not company:
            return False
        ...
        for warehouse in warehouses:
            for product in products:
                ...
                product_mrp = False
                if (
                    product.product_tmpl_id._fields.get("bom_count", False)
                ...
                inventory_in_moves = self.stock_move_search(
                    ...
                    origin_usage="internal",
                    warehouse=warehouse,
                )
                sales_moves = round(
                    (
                        sum(sales_out_moves.mapped("product_uom_qty"))
                        - sum(sales_in_moves.mapped("product_uom_qty"))
                    ),
                    0,
                )
                purchase_moves = round(
                    (
                        ...
                        - sum(purchase_out_moves.mapped("product_uom_qty"))
                    ),
                    0,
                )
                inventory_moves = round(
                    (
                        ...
                        - sum(inventory_in_moves.mapped("product_uom_qty"))
                    ),
                    0,
                )
                if product_mrp:
                    production_moves = round(
                        (
                            ...
                            - sum(
                                production_in_moves.mapped("product_uom_qty")
                            )
                        ),
                        0,
                    )
                ...
                stats = self.calculate_rotation(
                    ...
                    stock_init=qty_stock_init,
                    stock_end=qty_stock_end,
                    is_mrp=product_mrp,
    ...
    def compute_stock_rotation_day(
        ...
        while number_days > -1:
            self.compute_quantities_by_date(
                date_from=compute_date, date_to=compute_date, company=company
            )
            number_days -= 1
