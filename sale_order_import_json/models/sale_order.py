...
class SaleOrder(models.Model):
    ...
    def check_default_selection_field(self, content, key, record):
        return (
            record._fields[key].default(record._name)
            if (content[key] not in dict(record._fields[key].selection))
            else (content[key])
        )
    ...
    def json_import(self, json_content):
        ...
        def partner_get_or_create(data):
            ...
            if len(partners) > 1:
                raise ValidationError(
                    _(
                        'Multiple partners with same email "%s"'
        ...
        def partner_shipping_get_or_create(data):
            ...
            if len(partners) > 1:
                raise ValidationError(
                    _(
                        'Multiple partners shipping "%s" with same info'
                        % (json_content["partner"]["email"])
                    )
                )
            return partners
        ...
        if json_content.get("date_planned", False):
            user = self.env.context.get("login_user", False) or self.env[
                "res.users"
            ].browse(self.env.context.get("uid"))
        ...
        for line in json_content["order_line"]:
            ...
            if not products:
                supplierinfo = self.env["product.supplierinfo"].search(
                    [("product_code", "=", line["default_code"])], limit=1
                )
                if supplierinfo:
                    products = (
            ...
            line_total_taxed = float(line["price_unit_taxed"]) * float(
            ...
            if (
                line.discount != 100
                and abs(
                    line_total_taxed - (line.price_tax + line.price_subtotal)
                )
                > 0.6
            ):
                raise ValidationError(
                    _(
                        "Price with taxes does not match "
            ...
            for invoice in sale.invoice_ids:
                ...
                if json_content["state"] == "confirmed":
                    ...
                    if journal.type not in ["bank", "cash"]:
                        raise ValidationError(
                            _(
                                'The journal "%s" must be the type bank or '
                                "cash"
                            )
                            % journal.name
                        )
