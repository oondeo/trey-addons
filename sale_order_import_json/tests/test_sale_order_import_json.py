...
class TestSaleOrderImportJson(HttpCase):
    ...
    def test_sale_order_confirmed(self):
        ...
        self.assertEqual(
            sales.partner_id.street, self.data_json["partner"]["street"]
        )
        self.assertEqual(
            sales.partner_id.email, self.data_json["partner"]["email"]
        )
        ...
        total_product1 = self.data_json["order_line"][0][
            "price_unit_taxed"
        ] * (self.data_json["order_line"][0]["product_uom_qty"])
        total_product2 = self.data_json["order_line"][1][
            "price_unit_taxed"
        ] * (self.data_json["order_line"][1]["product_uom_qty"])
        self.assertEqual(sales.amount_total, total_product1 + total_product2)
    ...
    def test_payment_transaction_error_not_journal_found(self):
        ...
        with self.assertRaises(exceptions.ValidationError) as result:
            self.env["sale.order"].json_import(self.data_json)
        self.assertEquals(
            'Journal with name "Unknow" not found', result.exception.name
        )
    ...
    def test_error_product_barcode_not_exist(self):
        ...
        with self.assertRaises(exceptions.ValidationError) as result:
            self.env["sale.order"].json_import(self.data_json)
        self.assertEqual(
            result.exception.name, "Product with code %s not exist" % barcode
        )
