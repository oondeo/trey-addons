...
class SaleOrder(models.Model):
    ...
    def _action_confirm(self):
        ...
        for sale in self:
            ...
            if not pickings and qty_negative_lines:
                ...
                raise UserError(
                    _("No stock picking has been generated, check it.")
                )
            if len(pickings) > 1:
                raise UserError(
                    _(
                        "When the '%s' option is checked, there should only "
                        "be one stock picking generated."
                    )
                    % option_name
                )
    ...
    def get_option_name(self, sale):
        return (
            sale.is_sale_deposit
            and sale._fields["is_sale_deposit"].string
            or sale.is_inventory_deposit
            and sale._fields["is_inventory_deposit"].string
            or ""
        )
