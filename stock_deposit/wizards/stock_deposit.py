...
class StockDeposit(models.TransientModel):
    ...
    def process_line_sale(self, line, sale_lines, sale):
        ...
        qty_available = (
            sale_line
            and sale_line.product_id.with_context(
                location=line.location_src_id.id
            ).qty_available
    ...
    def process_line_return_customer_stock(
        ...
        if not picking_customer:
            picking_customer = self.create_picking_return(
        ...
        if not picking_stock:
            picking_stock = self.create_picking_return(
                line,
                line.deposit_id.location_id,
                line.deposit_id.warehouse_id.lot_stock_id,
            )
    ...
    def create_inventory(self, location, product, quantity):
        ...
        inventory = self.env["stock.inventory"].create(
            {
                "name": _("%s from move stock deposit wizard")
...
class StockDepositLine(models.TransientModel):
        ...
        for line in self:
            ...
            if deposit_lines:
                raise ValidationError(
                    _("There can only be one line for each product.")
                )
    ...
    def onchange_qty(self):
        ...
        elif self.force_inventory and self.ttype in [
            "sale",
            "sale_return_stock",
            "inventory",
        ]:
    ...
    def check_qty(self):
        ...
        for line in lines_without_stock:
            msg += _(
                ...
                "'%s'."
            ) % (line.product_id.default_code, line.qty_theorical, line.qty)
        msg += _(
