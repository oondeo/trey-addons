...
class PurchaseOrder(models.Model):
        ...
        for order in self:
            if order.is_ede_send:
                raise exceptions.Warning(
                    _("Purchase Order it's Already Send.")
                )
            ...
            if self.customer_shipping_id:
                ...
                address = {
                    ...
                    % (
                        ...
                        and self.customer_shipping_id.state_id.name
                        or "",
                    ),
                    "CountryCode": (
                        self.customer_shipping_id.country_id
                        and self.customer_shipping_id.country_id.code
                        or "ES"
                    ),
                }
            ...
            payload = {
                ...
                "OrderMemberCustomerID": self.ede_client_order_ref
                or self.origin
                or "",
                "Department": "Purchase",
                "CommissionID": "",
            ...
            if not response:
                msg = _(
                    "Error Send Purchase Order<p><ul><li><b>Message: "
            ...
            if response[0].find("Severity").text != "S":
                msg = _(
                    ...
                    response[0].find("ID").text,
                    response[0].find("Text").text,
                )
                self.message_post(body=msg)
                return
            self.write(
                {
                    "ede_document_id": response
                    and response[0].find("ID").text,
                    "is_ede_send": True,
                    "ede_state": "A",
            ...
            msg = _(
                "Purchase Order is Send <p><ul><li><b>ID:</b> %s</li>"
    ...
    def ede_check_status(self, orders):
        ...
        for order in orders:
            ...
            if ede_status == "A":
                continue
            if ede_status == "C" and order.ede_workflow_state in (
                "email",
                "done",
            ):
                order.ede_state = "C"
            ...
            for item in items:
                ...
                default_code = "%s%s" % (
                    start_code,
                    item.find("ProductOriginID").text[2:],
                )
                ...
                sale_line = order.sale_order_id.mapped("order_line").filtered(
                ...
                for tracking in res.findall(".//Tracklist/Item"):
                    if (
                        tracking.find("ItemNumber").text
                        == item.find("ItemNumber").text
                    ):
            ...
            if not order.customer_shipping_id:
                _log.info(
                    "Fail Purchase Order: %s Error: Not Customer "
            ...
            if not to_send_email:
                _log.info(
                    "Not Sales Picking to Send Email Purchase: %s" % order.name
                )
                continue
        ...
        for picking in pickings:
            ...
            for line in lines:
                ...
                move = picking.mapped("move_ids_without_package").filtered(
                    lambda m: m.purchase_line_id.id == line["purchase_line"].id
                )
                if not move:
                    continue
                if len(move) > 1:
            ...
            comment = _(
                ...
                % (
                    order.ede_client_order_ref,
                    order.ede_date_delivered,
            ...
            _log.info(
                "Confirming Picking: %s to Purchase: %s"
                % (picking.name, order.name)
            )
            picking.button_validate()
            picking.action_done()
            _log.info(
                "Confirmed Picking: %s to Purchase: %s"
                % (picking.name, order.name)
            )
            if status == "B":
    ...
    def ede_confirm_sale_order(self, order=None, lines=None, status=None):
        ...
        pickings = order.sale_order_id.mapped("picking_ids").filtered(
            lambda p: p.state
            in ("assigned", "partially_available", "waiting", "confirmed")
    ...
    def ede_picking_send_email(self, data, status=None):
        ...
        ede_user = (
            ...
            and self.env.user.company_id.ede_user_id
            or None
        )
        ...
        try:
            ...
            _log.info(
                "ERROR Send Email from Picking: %s and Sale: %s"
        ...
        _log.info(
            "Send Email from Picking: %s and Sale: %s"
            % (picking.name, purchase_order.sale_order_id.name)
        )
        if status == "B":
    ...
    def ede_ftp_invoice(self, orders=None):
        def data_header_info(xml=None):
            ...
            data = {
                ...
                "supplier_order_id": header["ORDER_HISTORY"][
                    "SUPPLIER_ORDER_ID"
                ],
            }
        ...
        def data_line_info(item=None):
            ...
            return {
                ...
                "item_order_ref": item["SUPPLIER_ORDER_REFERENCE"][
                    "SUPPLIER_ORDER_ID"
                ].strip(),
                ...
                "item_description": item["PRODUCT_ID"][
                ...
                data = self.env["purchase.order.line"].search(
                    [
                        ...
                        (
                            "product_id.barcode",
                            "=",
                            item["PRODUCT_ID"]["bmecat:INTERNATIONAL_PID"],
                        ),
                    ]
                )
            if not data:
        ...
        for file_name, file_info in files:
            ...
            if exist_invoice:
                _log.info(
                    "Invoice: %s is already created with id: %s"
                    % (
                ...
                self.env["account.invoice.ede.log.line"].create(
                    {
                        ...
                        "state": "done",
                        "log": line_msg,
                        "supplier_invoice_id": exist_invoice
                        and exist_invoice[0].id,
                    }
                )
            ...
            for item in items:
                ...
                if not ede_line:
                    line_msg += (
                        _("No found line for item: %s\n")
                        % item["PRODUCT_ID"]["bmecat:DESCRIPTION_SHORT"]
                    )
                    line_fail = True
                    continue
            ...
            invoice.mapped("invoice_line_ids").filtered(
                lambda l: l.purchase_line_id.id not in order_line_ids
            ).unlink()
