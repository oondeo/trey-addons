...
class TestProductSupplierinfoForCustomerCopy(common.TransactionCase):
    ...
    def test_copy_one_customerinfo(self):
        ...
        wizard = (
            self.env["product.customerinfo.copy"]
            .with_context(active_ids=product_customerinfo_1.id)
            .create(
                {
        ...
        self.assertEqual(wizard_line.date_end, product_customerinfo_1.date_end)
    ...
    def test_copy_multiple_customerinfo(self):
        ...
        self.assertEqual(
            product_customerinfos[0].product_id.id, self.product_1.id
        )
        ...
        self.assertEqual(
            product_customerinfos[1].product_id.id, self.product_2.id
        )
        ...
        wizard = (
            self.env["product.customerinfo.copy"]
            .with_context(active_ids=product_customerinfos.ids)
            .create(
                {
                    "partner_id": self.partner_2.id,
                }
