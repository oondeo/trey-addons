...
class CustomerPortal(CustomerPortal):
    ...
    def _prepare_portal_layout_values(self):
        ...
        order_count = SaleOrder.search_count(self._get_sale_order_domain())
        return_count = SaleOrder.search_count(self._get_sale_return_domain())
        values.update(
            {
    ...
    def portal_my_orders(
        ...
        res.qcontext.update(
            {"orders": orders.sudo(), "pager": pager, "search": search}
        )
        return res
        ...
        for i, line in enumerate(lines):
            ...
            if val < 0:
                errors.setdefault(
                    ...
                    {
                        "msg": (
                            _("The amount to be returned cannot be negative")
                        ),
                        "product": line,
                    },
                )
    ...
    def print_address_label(self, order_id, access_token=None, **kw):
        ...
        pdf = (
            request.env.ref("website_rma.action_report_address_label")
            .sudo()
            .render_qweb_pdf([order.id])[0]
        )
        pdfhttpheaders = [
    ...
    def portal_order_rma(self, order, access_token=None, **post):
        ...
        for order_line in order.order_line:
            ...
            if (
                product_id.type in order_line_obj._returnable_product_types()
    ...
    def portal_return_page(self, return_id, **kw):
        ...
        return request.render(
            "website_rma.sale_return_portal_template", values
        )
