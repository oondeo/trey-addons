...
class StockWarehouseOrderpoint(models.Model):
    ...
    @api.depends(
        "product_location_qty_available_not_res", "product_min_qty_year"
    )
    def compute_product_suggested_qty(self):
        for op in self:
            op.product_suggested_qty = (
                op.product_min_qty_year - op.virtual_location_qty
            )
        ...
        for op in self:
            ...
            op.product_min_qty_year = round(
                (
                    ...
                    - sum(in_moves.mapped("product_uom_qty"))
                )
                / 12,
                0,
            )
    ...
    def compute_copy_product_suggested_qty(self):
        for op in self:
            qty = (
                op.product_suggested_qty
                if op.product_suggested_qty > 0
                else 0.00
            )
            if (
                op.product_id.seller_ids
                and op.product_id.seller_ids[0].min_qty
                and qty > 0
            ):
                qty = (
                    math.ceil(qty / op.product_id.seller_ids[0].min_qty)
