...
class StockPicking(models.Model):
    ...
    def action_send_confirmation_email(self):
        ...
        res["context"]["default_template_id"] = self.env.ref(
            "print_formats_picking.mail_delivery_valued_confirmation"
        ).id
        return res

    @api.depends(
        "move_ids_without_package", "move_ids_without_package.move_line_ids"
    )
    def _compute_qty_total(self):
        for picking in self:
            if picking.state == "done":
                picking.qty_total_done = sum(
                    picking.move_line_ids.filtered(
                        lambda m: m.product_id.add_to_sum_qty
                    ).mapped("qty_done")
                )
            else:
                picking.qty_total = sum(
                    picking.move_line_ids.filtered(
                        lambda m: m.product_id.add_to_sum_qty
