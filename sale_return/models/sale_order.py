...
class SaleOrder(models.Model):
    ...
    def _compute_is_returnable(self):
        for order in self:
            ...
            order.returnable_date_to = (
                order.order_line
                and max([line.returnable_date for line in order.order_line])
                or False
            )
    ...
    def action_invoice_create(self, grouped=False, final=False):
        ...
        for invoice in self.env["account.invoice"].browse(invoice_ids):
            ...
            new_invoice = invoice.copy(
                {"type": "out_refund", "invoice_line_ids": False}
            )
    ...
    def _compute_sale_order_return_count(self):
        for sale in self:
            returns = self.env["sale.order"].search(
                [("parent_sale_order", "=", sale.id), ("is_return", "=", True)]
            )
