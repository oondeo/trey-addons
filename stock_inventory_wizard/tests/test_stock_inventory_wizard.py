...
class TestStockInventoryWizard(common.TransactionCase):
    ...
    def test_create_stock_inventory_one_product(self):
        wizard = (
            self.env["stock.inventory.product"]
            .with_context(active_ids=self.product_01.id)
            .create(
                {
                    "name": "Wizard test",
                }
        ...
        self.assertEqual(wizard.line_ids[0].qty_available, 0)
    ...
    def test_create_stock_inventory_multiple_products(self):
        wizard = (
            self.env["stock.inventory.product"]
            .with_context(active_ids=[self.product_01.id, self.product_02.id])
            .create(
                {
                    "name": "Wizard test",
                }
        ...
        self.assertEqual(wizard.line_ids[0].qty_available, 0)
        self.assertEqual(wizard.line_ids[1].qty_available, 0)
    ...
    def test_create_stock_inventory_change_location(self):
        wizard = (
            self.env["stock.inventory.product"]
            .with_context(active_ids=self.product_01.ids)
            .create(
                {
                    "name": "Wizard test",
                }
    ...
    def test_different_theoretical_qty_in_different_location_and_confirm(self):
        ...
        self.assertNotEqual(
            self.product_01.with_context(
                location=location_01.id
            ).qty_available,
            self.product_01.with_context(
                location=location_02.id
            ).qty_available,
        )
        self.assertEqual(
