...
class ImportTemplateProductVariant(models.TransientModel):
    ...
    def categ_id_get_or_create(self, category):
        def _get_or_create_parent(cat_name, parent_id):
            ...
            if len(categs) > 1:
                errors.append(
                    _("More than one category found for %s.") % cat_name
                )
            if not categs:
    ...
    def _get_uom(self, uom_name):
        ...
        if not uom_name:
            ...
            return None, errors
        uoms = self.env["uom.uom"].search([("name", "=", uom_name)])
        if len(uoms) > 1:
            errors.append(
                _("More than one product uom found for %s.") % uom_name
            )
        if not uoms:
            errors.append(
                _(
    ...
    def product_brand_id_get_or_create(self, brand_name):
        ...
        if len(brands) > 1:
            errors.append(
                _("More than one product brand found for %s.") % brand_name
            )
        if not brands:
            errors.append(
                _(
    ...
    def find_products(self, tmpls, products, attr_values, variant_found):
        for product in tmpls.product_variant_ids:
            if variant_found is True:
                break
            perms = permutations(product.attribute_value_ids.mapped("name"))
    ...
    def import_file(self, simulation=True):
        ...
        for index, row in df.iterrows():
            ...
            product_tmpl_data, errors = self.get_product_tmpl_values(df, row)
            for error in errors:
                all_errors.append((row_index, [error]))
            ...
            try:
                ...
                if tmpls:
                    ...
                    image_variant = None
                    if "image_variant" in data:
                        images, warns = self.images_get(data["image_variant"])
                        for warn in warns:
                            all_warns.append((row_index, [warn]))
                    ...
                    attr_values_no_variant, errors = self.with_context(
