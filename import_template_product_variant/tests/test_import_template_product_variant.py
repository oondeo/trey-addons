...
class TestImportTemplateProduct(TransactionCase):
        ...
        wizard = self.env["import.file"].create(
            {
                "template_id": self.env.ref(
                    "import_template_product_variant.template_product_variant"
                ).id,
                "file": file,
                ...
                "attribute_line_ids": [
                    (
                        ...
                        0,
                        {
                            "attribute_id": self.attr_color.id,
                            "value_ids": [
                                (6, 0, self.attr_color.value_ids.ids)
                            ],
                        },
                    ),
                    (
    ...
    def test_import_write_diff_variants_exists(self):
                ...
                "attribute_line_ids": [
                    ...
                    (
                        ...
                        0,
                        {
                            "attribute_id": self.attr_lenght.id,
                            "value_ids": [
                                (6, 0, self.attr_lenght.value_ids.ids)
                            ],
                        },
                    ),
                ],
        ...
        for i in range(1, wizard.total_error + 1):
            self.assertIn(
                _("%s: %s" % (i + 1, msg_error)), wizard.line_ids[i - 1].name
            )
        product_tmpls_1 = self.env["product.template"].search(
            [
                ("product_tmpl_code", "=", "PRODTMPL1TEST"),
                ...
                "attribute_line_ids": [
                    ...
                    (
                        ...
                        0,
                        {
                            "attribute_id": self.attr_gender.id,
                            "value_ids": [
                                (6, 0, self.attr_gender.value_ids.ids)
                            ],
                        },
                    ),
                ],
    ...
    def test_import_create_three_attributes_exists(self):
        ...
        self.assertEquals(wizard.total_error, 0)
        self.assertIn(
            _("2: Product template 'Template 1' not found, will be created."),
            wizard.line_ids[0].name,
        )
        self.assertIn(
            _("3: Product template 'Template 1' not found, will be created."),
            wizard.line_ids[1].name,
        )
        self.assertIn(
            _("4: Product template 'Template 1' not found, will be created."),
            wizard.line_ids[2].name,
        )
        self.assertIn(
            _("5: Product template 'Template 2' not found, will be created."),
            wizard.line_ids[3].name,
        )
        self.assertIn(
            _("6: Product template 'Template 2' not found, will be created."),
            wizard.line_ids[4].name,
        )
        self.assertIn(
            _("7: Product template 'Template 2' not found, will be created."),
    ...
    def test_import_write_three_attributes_exists(self):
        ...
        self.assertEquals(variant_WL.barcode, "4050119164106")
        remainder_variants = (
            product_tmpls_1.product_variant_ids
            - variant_WS
            - variant_WM
            - variant_WL
        )
    ...
    def test_import_create_new_columns(self):
        ...
        self.assertIn(
            _("5: Product template 'Template 1' not found, will be created."),
            wizard.line_ids[3].name,
        )
        self.assertIn(
            _("6: Product template 'Template 1' not found, will be created."),
            wizard.line_ids[4].name,
        )
        self.assertIn(
            _("7: Product template 'Template 1' not found, will be created."),
            wizard.line_ids[5].name,
        )
        product_tmpls_1 = self.env["product.template"].search(
            [
