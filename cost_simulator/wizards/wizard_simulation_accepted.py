...
class WizardCostAccepted(models.TransientModel):
    ...
    def button_ok(self):
        ...
        if active_id:
            ...
            analytic_id = cost_id.company_id.project_template_id.copy()
            analytic_id.partner_id = cost_id.partner_id.id
            analytic_id.name = "[%s]%s" % (
                cost_id.simulation_number,
                cost_id.name,
            )
                    ...
                    sale_id = self.env["sale.order"].create(
                        {
                            "partner_id": cost_id.partner_id.id,
                            "company_id": cost_id.company_id.id,
                            "type_id": cost_id.company_id.project_sale_type_id.id
                            or None,
                            "client_order_ref": "%s-%s"
                            % (cost_id.simulation_number, cost_id.name),
                        }
    ...
    def _convert_qty_company_hours(self, product, line):
        ...
        if (
            product.uom_id.id != company_time_uom_id.id
            and product.uom_id.category_id.id
            == company_time_uom_id.category_id.id
        ):
            planned_hours = self.env["product.uom"]._compute_qty(
                product.uom_id.id, line.quantity_sale, company_time_uom_id.id
            )
        else:
