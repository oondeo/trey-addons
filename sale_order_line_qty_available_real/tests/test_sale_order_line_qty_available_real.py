...
class TestSaleOrderLineQuantityAvailableReal(common.TransactionCase):
    ...
    def test_sale_order_line_qty_available_real(self):
        ...
        quant._update_available_quantity(self.product, self.location, quantity)
        ...
        self.assertEqual(self.product.qty_available, quantity)
        self.assertEqual(
            self.product.qty_available_real, quantity - product_uom_qty
        )
        ...
        picking.action_done()
        self.assertEqual(
            self.product.qty_available, self.product.qty_available_real
        )
    ...
    def test_sale_order_line_not_stock_enough(self):
        ...
        self.assertEqual(
            self.product.qty_available_real, order_line.qty_available_real
        )
        message = order_line._onchange_product_id_check_availability()
        self.assertEqual(
            message["warning"]["title"], _("Not enough inventory!")
        )
        sale.action_confirm()
        self.assertEqual(
            self.product.qty_available_real, quantity - product_uom_qty
        ...
        sale = self.env["sale.order"].create(
            {
                ...
                "order_line": [
                    (
                        ...
                        0,
                        {
                            "product_id": product_tmpl.product_variant_ids[
                                0
                            ].id,
                            "price_unit": 100,
                            "product_uom_qty": product_uom_qty,
