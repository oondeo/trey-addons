...
class SaleOrder(models.Model):
    ...
    def _process_invoice_line(
        self, invoice_line, sale, sale_product_components, sale_line_components
    ):
        qty_sale_line = sale_line_components.filtered(
            lambda ln: ln.product_id == invoice_line.product_id
            and ln in invoice_line.sale_line_ids
        ...
        qty_delivered = sum(
            moves_delivered.filtered(
                lambda m: m.product_id == invoice_line.product_id
                and m.sale_line_id in invoice_line.sale_line_ids
            ).mapped("product_uom_qty")
        )
        qty_to_split = qty_delivered - qty_sale_line
        to_split = qty_to_split > 0 and invoice_line.quantity > qty_to_split
        if to_split:
