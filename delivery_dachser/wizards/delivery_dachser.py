...
class DeliveryDachser(models.TransientModel):
    ...
    def _get_default_dachser_product(self):
        ...
        if params:
            ...
            if (
                ...
                and (picking.carrier_id.delivery_type == "dachser")
            ):
    ...
    def action_create_dachser_file(self):
        ...
        file_content = string_format % (
            ...
            and self.picking_id.partner_id.name[:32].upper()
            or "",
            self.picking_id.partner_id.street
            and self.picking_id.partner_id.street[:40].upper()
            or "",
            self.picking_id.partner_id.city
            and self.picking_id.partner_id.city[:27].upper()
            or "",
            self.picking_id.partner_id.zip
            and self.picking_id.partner_id.zip[:12]
            or "",
            ...
            and self.picking_id.delivery_note[:38]
            or "",
            self.picking_id.delivery_note
            and self.picking_id.delivery_note[:38]
            or "",
            ...
            and self.picking_id.partner_id.phone[:14]
            or "",
            self.picking_id.partner_id.email
            and self.picking_id.partner_id.email[:80]
            or "",
