...
class TestRestriction(TransactionCase):
    ...
    def test_check_access_warehouse_01(self):
        self.warehouse1.with_user(self.manager_warehouse).browse(
            self.warehouse1.id
        ).name
        with self.assertRaises(AccessError):
            self.warehouse1.with_user(self.user_warehouse).browse(
                self.warehouse1.id
            ).name
    ...
    def test_check_access_warehouse_03(self):
        ...
        with self.assertRaises(AccessError):
            self.warehouse2.with_user(self.user_warehouse).browse(
                self.warehouse2.id
            ).name
    ...
    def test_check_access_picking_type_01(self):
        self.warehouse1.out_type_id.with_user(self.manager_warehouse).browse(
            self.warehouse1.out_type_id.id
        ).name
        with self.assertRaises(AccessError):
    ...
    def test_check_access_picking_type_03(self):
        ...
        with self.assertRaises(AccessError):
            self.warehouse2.out_type_id.with_user(self.user_warehouse).browse(
                self.warehouse2.out_type_id.id
            ).name
    ...
    def test_check_access_picking_01(self):
        ...
        with self.assertRaises(AccessError):
            self.picking1.with_user(self.user_warehouse).browse(
                self.picking1.id
            ).name
    ...
    def test_check_access_picking_03(self):
        ...
        with self.assertRaises(AccessError):
            self.picking2.with_user(self.user_warehouse).browse(
                self.picking2.id
            ).name
    ...
    def test_check_access_orderpoint_03(self):
        ...
        with self.assertRaises(AccessError):
            self.orderpoint_wh2.with_user(self.user_warehouse).browse(
                self.orderpoint_wh2.id
            ).name
    ...
    def test_check_access_location_03(self):
        ...
        with self.assertRaises(AccessError):
            location_wh2.with_user(self.user_warehouse).browse(
                location_wh2.id
            ).name
        location_wh2.with_user(self.manager_warehouse).browse(
            location_wh2.id
        ).name

    def test_check_access_inventory_01(self):
        self.inventory_wh1.with_user(self.manager_warehouse).browse(
            self.inventory_wh1.id
        ).name
        with self.assertRaises(AccessError):
    ...
    def test_check_access_inventory_line_01(self):
        self.inventory_wh1.line_ids[0].with_user(
            self.manager_warehouse
        ).browse(self.inventory_wh1.line_ids[0].id).product_id
        with self.assertRaises(AccessError):
            self.inventory_wh1.line_ids[0].with_user(
                self.user_warehouse
            ).browse(self.inventory_wh1.line_ids[0].id).product_id
    ...
    def test_check_access_inventory_line_02(self):
        ...
        self.inventory_wh1.line_ids[0].with_user(self.user_warehouse).browse(
            self.inventory_wh1.line_ids[0].id
        ).product_id
        self.inventory_wh1.line_ids[0].with_user(
            self.manager_warehouse
        ).browse(self.inventory_wh1.line_ids[0].id).product_id
    ...
    def test_check_access_inventory_line_03(self):
        ...
        with self.assertRaises(AccessError):
            self.inventory_wh2.line_ids[0].with_user(
                self.user_warehouse
            ).browse(self.inventory_wh2.line_ids[0].id).product_id
        self.inventory_wh2.line_ids[0].with_user(
            self.manager_warehouse
        ).browse(self.inventory_wh2.line_ids[0].id).product_id

    def test_check_access_account_journal_01(self):
        self.check_journal.with_user(self.manager_warehouse).browse(
            self.check_journal.id
        ).name
        with self.assertRaises(AccessError):
            self.check_journal.with_user(self.user_warehouse).browse(
                self.check_journal.id
            ).name
    ...
    def test_check_access_account_journal_03(self):
        ...
        with self.assertRaises(AccessError):
            self.check_journal2.with_user(self.user_warehouse).browse(
                self.check_journal2.id
            ).name
    ...
    def test_check_access_invoice_journal_01(self):
        self.sale_journal.with_user(self.manager_warehouse).browse(
            self.sale_journal.id
        ).name
        with self.assertRaises(AccessError):
            self.sale_journal.with_user(self.user_warehouse).browse(
                self.sale_journal.id
            ).name
    ...
    def test_check_access_invoice_journal_03(self):
        ...
        with self.assertRaises(AccessError):
            self.sale_journal2.with_user(self.user_warehouse).browse(
                self.sale_journal2.id
            ).name
    ...
    def test_check_update_locations_when_create_location_01(self):
        ...
        location_test = (
            ...
            .with_user(self.manager_wh_salesman)
            .create(
                {
                    "name": "Location test",
