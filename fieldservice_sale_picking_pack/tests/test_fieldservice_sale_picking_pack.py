...
class TestFieldserviceSalePickingPack(common.TransactionCase):
    ...
    def test_fsm_track_line_service_installs_and_product_dependend(self):
        ...
        self.assertFalse(line_product_A.sale_line_fsm_id)
        wizard = (
            self.env["sale.order.relate_to_installations"]
            .with_context(
                {
                    "active_ids": sale.ids,
                    "active_id": sale.id,
        ...
        wizard.line_ids[
            0
        ].sale_line_fsm_id = line_product_installation_line_1.id
        ...
        fsm_install_1 = sale.fsm_order_ids.filtered(
            lambda f: f.move_internal_ids.mapped("product_id")
            == components_install_1
        )
