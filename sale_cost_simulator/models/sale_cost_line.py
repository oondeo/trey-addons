...
class SaleCostSimulatorLine(models.Model):
    ...
    def compute_this(self):
        ...
        for line in self:
            ...
            line.total_untaxed = (
                sum(c.total_untaxed for c in line.child_ids)
                + line.amount_untaxed
            )
            line.total_tax = (
    ...
    def _get_display_price(self, product):
        if self.pricelist_id.discount_policy == "with_discount":
            return product.with_context(
                pricelist=self.pricelist_id.id, uom=self.uom_id.id
            ).price
        ...
        final_price, rule_id = self.pricelist_id.with_context(
            ...
            product or self.product_id, self.quantity or 1.0, self.partner_id
        )
        base_price, currency = self.with_context(
            product_context
        )._get_real_price_currency(
            product, rule_id, self.quantity, self.uom_id, self.pricelist_id.id
        )
