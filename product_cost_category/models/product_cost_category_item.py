...
class ProductCostCategoryItem(models.Model):
    ...
    formula = fields.Text(
        string="Formula", required=True, default="standard_price / "
    )
    ...
    def check_standard_price_range(self):
        ...
        for item in self:
            if float_is_zero(
                item.from_standard_price, precision
            ) and float_is_zero(item.to_standard_price, precision):
                raise ValidationError(
                    _("From Standard Price and To Standard Price is zero")
                )
            if item.to_standard_price <= item.from_standard_price:
                raise ValidationError(
                    _("From Standard Price greater To Standard Price")
                )
    ...
    def check_formula(self):
        for item in self:
            try:
                ...
                item.is_ok = False
                raise ValidationError(
                    _("Formula Error: No Return Float Value")
                )
