...
class StockDepositImportJson(http.Controller):
    ...
    def get_product(self, data):
        ...
        if not products:
            raise ValidationError(_("Product with code %s not exist" % data))
        if len(products) > 1:
            raise ValidationError(_("Many products with same code %s" % data))
        return products
    ...
    @http.route(["/stock_deposit/import"], type="json", auth="user")
