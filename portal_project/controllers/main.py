...
class CustomerPortal(CustomerPortal):
    ...
    def portal_my_project(self, project_id=None, access_token=None, **kw):
        ...
        year_from = fields.Datetime.from_string(
            project.date_start and project.date_start or project.create_date
        ).year
        searchbar_filters = {
        ...
        for timesheet in timesheets:
            if filterby != "all":
                key = "%s / %s" % (
                    ...
                    str(
                        fields.Datetime.from_string(timesheet.date).month
                    ).zfill(2),
                )
            else:
                key = "%s" % (fields.Datetime.from_string(timesheet.date).year)
            values.setdefault(key, 0)
            values[key] += timesheet.unit_amount
        values = OrderedDict(sorted(values.items()))
        res.qcontext["searchbar_filters"] = OrderedDict(
            ...
            [
                {
                    ...
                    "values": [
                        {"text": k, "count": v} for k, v in values.items()
                    ],
                }
            ]
        )
