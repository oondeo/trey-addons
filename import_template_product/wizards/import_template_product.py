...
class ImportTemplateProduct(models.TransientModel):
    ...
    def attributes_line_get(self, df, row):
        ...
        for c in cols:
            if row[c] is None:
                continue
            attrs[c.replace("attribute:", "").strip()] = list_attr_values(
                row[c]
            )
        res = []
        errors = []
        ...
        for attr, vals in attrs.items():
            ...
            if len(attrs) > 1:
                errors.append(
                    _("More than one attribute found for %s.") % attr
                )
                continue
            res.append(
                (
                    ...
                    0,
                    {
                        ...
                        "value_ids": [
                            (
                                ...
                                0,
                                [
                                    attr_value_id_get_or_create(attrs, v)
                                    for v in vals
                                ],
                            )
                        ],
