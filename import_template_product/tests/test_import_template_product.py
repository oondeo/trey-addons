...
class TestImportTemplateProduct(TransactionCase):
        ...
        wizard = self.env["import.file"].create(
            {
                "template_id": self.env.ref(
                    "import_template_product.template_product"
                ).id,
                "file": file,
    ...
    def test_import_create_with_categ_base_variants_ok(self):
        ...
        self.assertEquals(
            all(product_tmpl_1.product_variant_ids.mapped("standard_price")),
            True,
        )
        self.assertEquals(
        ...
        self.assertEquals(
            all(product_tmpl_2.product_variant_ids.mapped("standard_price")),
            True,
        )
        self.assertEquals(
    ...
    def test_import_categ_empty_error(self):
        ...
        self.assertIn(
            _("3: Category None not found."), wizard.line_ids[0].name
        )
        self.assertIn(
            _(
    ...
    def test_import_create_with_attributes_variants_ok(self):
        ...
        attr_val_obj.create({"attribute_id": attr.id, "name": "S"})
        attr_val_obj.create({"attribute_id": attr.id, "name": "M"})
        fname = self.get_sample("sample_categ_base_variants_ok.xlsx")
        file = base64.b64encode(open(fname, "rb").read())
        wizard = self.env["import.file"].create(
            {
    ...
    def test_attr_and_categ_duply_error(self):
        ...
        attr_val_obj.create({"attribute_id": attr2.id, "name": "M"})
        ...
        self.assertIn(
            _("2: More than one category found for Categ 1."),
            wizard.line_ids[0].name,
        )
        self.assertIn(
            _("2: More than one attribute found for Talla."),
            wizard.line_ids[1].name,
        )
        self.assertIn(
            _("4: More than one attribute found for Talla."),
    ...
    def test_import_create_new_fields_no_variants_ok(self):
        ...
        self.assertEquals(len(product_tmpls_1), 1)
        self.assertIn(
            _("Description for prod 1."), product_tmpls_1.description_sale
        )
        ...
        self.assertEquals(len(product_tmpls_2), 1)
        self.assertIn(
            _("Description for prod 2."), product_tmpls_2.description_sale
        )
    ...
    def test_new_fields_error(self):
        ...
        self.assertIn(
            _(
                ...
                "is no defined function to convert it."
            ),
            wizard.line_ids[1].name,
        )
        self.assertIn(
    ...
    def test_new_fields_ean_duply_error(self):
        ...
        self.assertEquals(wizard.total_error, 1)
        self.assertIn(
            _("Key (barcode)=(7501031311309) already exists."),
            wizard.line_ids[0].name,
        )
        self.assertIn(
            _(
    ...
    def test_image_error_create_ok(self):
        ...
        self.assertEquals(
            all(product_tmpl_4.product_variant_ids.mapped("standard_price")),
            True,
        )
        self.assertEquals(
    ...
    def test_import_two_files_write_existent_product_tmpl_variants(self):
        ...
        self.assertEquals(
            all(product_tmpl_3.product_variant_ids.mapped("standard_price")),
            True,
        )
        self.assertEquals(
        ...
        for product in product_tmpl_3.product_variant_ids:
            if product.attribute_value_ids.mapped("name") in (
                ["Azul", "M"],
                ["Azul", "L"],
            ):
                self.assertFalse(product.image_variant)
