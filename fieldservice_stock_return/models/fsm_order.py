...
class FsmOrder(models.Model):
    ...
    def create_picking(self, moves):
        ...
        for move in moves:
            ...
            qty_left = move.quantity_done - sum(
                moves_out.mapped("quantity_done")
            )
            if qty_left <= 0:
                continue
    ...
    def action_return_stock(self):
        for order in self:
            if order.stock_returned:
                raise UserError(
                    _("Fieldservice order stock already returned.")
                )
            if not order.stage_is_closed:
                raise ValidationError(
                    _("Fieldservice order must be in done or cancel state.")
                )
