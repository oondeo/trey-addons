...
class TestProductCreateOrderpointAuto(TransactionCase):
    def setUp(self):
        ...
        self.warehouse = (
            ...
            .search([])
            .filtered(lambda wh: wh.company_id.id == self.user_company.id)
        )
    ...
    def create_new_company(self):
        return self.env["res.company"].create({"name": "New test company"})
    ...
    def test_multicompany_create_type_service(self):
        ...
        self.warehouse = (
            self.env["stock.warehouse"]
            .search([])
            .filtered(lambda wh: wh.company_id.id == new_company.id)
        )
        self.assertEquals(len(self.warehouse), 1)
        new_company.warehouse_auto_orderpoint_ids = [
            (6, 0, self.warehouse.ids)
        ]
        self.assertEquals(len(new_company.warehouse_auto_orderpoint_ids), 1)
        new_user = self.create_new_user(new_company)
        product = (
            self.env["product.product"]
            .with_user(new_user.id)
            .create(
                {
                    "type": "service",
                    "company_id": False,
    ...
    def test_multicompany_create_type_product_several_warehouses(self):
        ...
        self.assertEquals(len(new_company.warehouse_auto_orderpoint_ids), 2)
        new_user = self.create_new_user(new_company)
        product = (
            self.env["product.product"]
            .with_user(new_user.id)
