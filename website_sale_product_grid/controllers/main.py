...
class WebsiteSale(WebsiteSale):
    ...
    def _get_cart_quantities(self, order=None):
        order = order or request.website.sale_get_order(force_create=1)
        cart_quantities = {}
    ...
    def _fill_grid(self, product_tmpls, main_attr):
        def get_attribute_values_sorted(values):
            return values.sorted(lambda v: v.attribute_id.sequence).mapped(
                "sequence"
            )
        ...
        for tmpl in product_tmpls:
            ...
            for variant in variants:
                ...
                key = "%s-%s" % (
                    tmpl.id,
                    "-".join([str(i) for i in value_row.ids]),
                )
    ...
    def product(self, product, category="", search="", **kwargs):
        ...
        if product.attribute_line_ids:
            ...
            res.qcontext["main_attribute_id"] = product.attribute_line_ids[
                0
            ].attribute_id
