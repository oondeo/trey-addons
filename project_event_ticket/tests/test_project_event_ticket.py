...
class TestProjectEvent(TransactionCase):
    ...
    def test_project_event_generator(self):
        product_ticket = self.env.ref("event_sale.product_product_event")
        ...
        self.assertEquals(
            len(project.event_ids.mapped("event_ticket_ids")), 12
        )
        self.assertEquals(
            self.date2str(project.event_ids[-1].date_begin), "2022/12/01 00:00"
        )
