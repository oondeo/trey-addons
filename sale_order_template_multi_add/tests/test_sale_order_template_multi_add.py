...
class TestSaleOrderTemplateMultiAdd(TransactionCase):
    ...
    def test_sale_permission(self):
        ...
        wizard_obj = self.env["sale.import.sale.order.template"].with_user(
            user
        )
    ...
    def test_conflict_sale_product_min_quantity_addon(self):
        ...
        if module.state != "installed":
            self.skipTest("Module sale_product_min_quantity not installed")
            return
        tmpl = self.create_sale_template("1")
    ...
    def test_product_contract(self):
        ...
        self.assertTrue(module)
        if module.state != "installed":
            self.skipTest("Module product_contract not installed")
            return
        ...
        self.assertEqual(
            line.recurring_rule_type, line.product_id.recurring_rule_type
        )
        self.assertEqual(
