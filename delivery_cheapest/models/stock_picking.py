    ...
    def assign_cheapest_delivery_carrier(self):
        ...
        if not carriers:
            raise ValidationError(
                _(
                    "No shipping methods found with the check for calculating "
        ...
        self.carrier_id = (
            prices_dict and (min(prices_dict, key=prices_dict.get)) or False
        )
        res = (
            self.carrier_id.rate_shipment(self.sale_id)
            if (self.carrier_id)
            else False
        )
