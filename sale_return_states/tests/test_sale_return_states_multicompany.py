...
class TestSaleReturnStatesMulticompany(common.TransactionCase):
    ...
    def picking_transfer(self, picking):
        picking.with_user(user=self.user).action_confirm()
        picking.with_user(user=self.user).action_assign()
        for move in picking.move_lines:
            move.quantity_done = move.product_uom_qty
        picking.with_user(user=self.user).action_done()

    def update_qty_on_hand(self, product, location, new_qty):
        wizard_model = self.env["stock.change.product.qty"]
        wizard = wizard_model.with_user(user=self.user).create(
            {
        ...
        wizard.with_user(user=self.user).change_product_qty()
        self.assertEquals(
    ...
    def test_sale_return_change_multicompany(self):
        ...
        sale_return.with_user(user=self.user).onchange_partner_id()
        vline = (
        ...
        vline.with_user(user=self.user).product_id_change()
        self.env["sale.order.line"].with_user(user=self.user).create(
            vline._convert_to_write(vline._cache)
        )
        self.assertTrue(sale_return)
        sale_return.with_user(user=self.user).action_confirm()
        ...
        self.assertEquals(
            self.product_1.with_context(
                location=self.scrap_location.id
            ).qty_available,
            10,
        )
        self.assertEquals(
