...
class WizCreateInvoice(models.TransientModel):
    ...
    def get_month_labels(self, trimester):
        ...
        for month in range(((trimester - 1) * 3) + 1, (trimester * 3) + 1):
            months_labels[month] = str.capitalize(
    ...
    def get_expedients(self, subvention, trimester, target_year):
        ...
        for account_move_line in subvention.account_move_line_ids:
            ...
            month = (
                (account_invoice and account_invoice.date_invoice)
                and account_invoice.date_invoice.month
                or -1
            )
            year = (
                (account_invoice and account_invoice.date_invoice)
                and account_invoice.date_invoice.year
                or -1
            )
            if year != target_year:
                continue
            ...
            if self.month_in_trimester(month, trimester):
                ...
                for (
                    invoice_line
                ) in account_move_line.invoice_id.invoice_line_ids:
    ...
    def _prepare_report_data(self):
        ...
        data = {
            ...
            "expedients": self.get_expedients(
                subvention,
                self.trimester,
                self.fiscal_year.date_from.year,
            ),
        }
        return data
