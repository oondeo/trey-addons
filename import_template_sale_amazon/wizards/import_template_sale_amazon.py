...
class ImportTemplateSaleAmazon(models.TransientModel):
    ...
    def _get_product_id(self, default_code):
        ...
        if not products:
            errors.append(_("The product '%s' does not exist.") % default_code)
            return None, errors
        ...
        data.update(
            {
                ...
                "price_unit": (
                    round(
                        df["price_unit"][index] / df["product_uom_qty"][index],
                        2,
                    )
                ),
    ...
    def get_partner_data(self, df, index):
        ...
        try:
            ...
            phone = (
                ...
                and int(
                    re.sub("[^0-9]", "", str(df["buyer-phone-number"][index]))
                )
                or ""
            )
        data = {
        ...
        for index, row in df.iterrows():
            ...
            if data_order.get("origin", None):
                ...
                if amazon_sale_number not in data_orders:
                    data_orders[amazon_sale_number] = {"order_line": []}
                if not data_line:
