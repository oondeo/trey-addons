...
class ProductTemplate(models.Model):
    ...
    def _get_searchable_fields(self, template):
        ...
        list_searchable_fields.append(
            " ".join(
                str(v)
                for v in template.product_variant_ids.mapped("default_code")
                if v is not False
            )
        )
    ...
    def _compute_searchable_text(self):
        for template in self:
            template.searchable_text = re.sub(
                "<.*?>", "", " ".join(self._get_searchable_fields(template))
            )
