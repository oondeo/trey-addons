...
class ProductBusinessUnit(models.Model):
    ...
    def _compute_sales(self):
        for unit in self:
            ...
            unit.sale_order_count = len(sale_lines.mapped("order_id"))
    ...
    def action_view_quotation_lines(self):
        ...
        action = self.env.ref(
            "sale_business_unit.sale_order_line_quotation_action"
        ).read()[0]
    ...
    def action_view_sale_lines(self):
        ...
        action = self.env.ref(
            "sale_business_unit.sale_order_line_sale_action"
        ).read()[0]
        action["domain"] = [("id", "in", lines.ids)]
        return action
    ...
    def action_view_invoice_lines(self):
        ...
        action = self.env.ref(
            "sale_business_unit.account_invoice_line_action"
        ).read()[0]
        action["domain"] = [("id", "in", lines.ids)]
        return action
    ...
    def action_view_sale(self):
        self.ensure_one()
        lines = self.env["sale.order.line"].search(
            [("product_id", "!=", False), ("product_id.unit_id", "=", self.id)]
        )
        sale_lines = lines.filtered(
            lambda l: l.order_id.state in ["sale", "done"]
        )
